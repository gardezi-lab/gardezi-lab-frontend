...
export default function BRV() {
  ...
  const getVoucherData = async () => {
    setLoading(true);
    try {  const res = await httpClient.get("/journal_vouchers?voucher_type=BRV");
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (isEdit && selectedVoucher) {
        await httpClient.put(`/bank_receipt_voucher/${selectedVoucher.id}`, formData);
      } else {
        await httpClient.post("/bank_receipt_voucher", formData);
      }
      getVoucherData();
    } catch (err) {
