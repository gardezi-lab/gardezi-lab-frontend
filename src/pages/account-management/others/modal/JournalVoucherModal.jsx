...
export default function JournalVoucherModal({ onSave, onCancel, voucher }) {
  ...
  const handleAddEntry = () => {
    if (!accountHead || (!dr && !cr)) return alert("Please fill Account Head and DR or CR");
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    if (!date || !narration || entries.length === 0) return alert("Please fill all fields");
    if (totalDr !== totalCr) return alert("Debit and Credit totals must be equal");
    ...
    <Container>
      <Form onSubmit={handleSubmit}>
          ...
          <Col>
            <Form.Group className="mb-3">
              ...
              >
                ...
                {accountList.map(a => (
                  <option key={a.id} value={a.id}>{a.name_head}</option>
                ))}
              </Form.Select>
            </Form.Group>
          </Col>
            ...
            <tbody>
              {entries.map((entry, i) => (
                <tr
                  ...
                  style={{ backgroundColor: totalDr !== totalCr ? "rgba(255,0,0,0.1)" : "transparent" }}
                >
                  <td>{entry.name_head}</td>
                  <td>{entry.dr.toFixed(2)}</td>
                  <td>{entry.cr.toFixed(2)}</td>
                  <td style={{ cursor: "pointer", color: "red" }} onClick={() => handleRemove(i)}>✕</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="text-center fw-bold" style={{ backgroundColor: totalDr !== totalCr ? "rgba(255,0,0,0.1)" : "transparent" }}>
                <td>Total</td>
        ...
        <div className="d-flex justify-content-end gap-2 align-items-center">
          <Button variant="secondary" className="secondary" onClick={onCancel} disabled={saving}> Cancel & Close</Button>
          <Button variant="primary" className="primary" type="submit" disabled={saving || totalDr !== totalCr}>
            {saving ? (
              <>
